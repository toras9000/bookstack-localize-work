...
return await Paved.ProceedAsync(noPause: Args.RoughContains("--no-pause"), async () =>
{
    ...
    var tokenExists = await mysql.QueryFirstAsync<long>("select count(*) from api_tokens where name = @name", param: new { name = settings.BookStack.Api.TokenName, });
    if (0 < tokenExists)
    {
    ...
    var secretHash = BCrypt.Net.BCrypt.HashPassword(settings.BookStack.Api.TokenSecret, hashSalt);
    var tokenParam = new
    {
        name = settings.BookStack.Api.TokenName,
        token_id = settings.BookStack.Api.TokenId,
        secret = secretHash,
        user_id = adminId,
